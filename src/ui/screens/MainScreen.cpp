...
void MainScreen::mount(lv_obj_t* parent)
{
    ...
    lv_obj_t* backgroundImage = lv_img_create(decorated_->bodySlot());
    backgroundImage_.attach(backgroundImage);
    lv_img_set_src(backgroundImage, &antares_bg);
    lv_obj_add_flag(backgroundImage, LV_OBJ_FLAG_IGNORE_LAYOUT);
    lv_obj_clear_flag(backgroundImage, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_center(backgroundImage);
}
